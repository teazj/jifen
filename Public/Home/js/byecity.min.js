...
Byecity.Common.Extend = {
    ...
    */
    CreateLiTab: function(tabID, contentID) {
        ...
        //if ($.cookie("Byecity_utm_medium") != null && $.cookie("Byecity_utm_medium") == "byecityunion" && $.cookie("WebAffiliate") != null)
        if (($.cookie("Byecity_utm_medium") != null && $.cookie("Byecity_utm_medium") == "byecityunion" && $.cookie("WebAffiliate") != null)
            || ($.cookie("Byecity_utm_source") != null && $.cookie("Byecity_utm_source") == "byecityunion" && $.cookie("WebAffiliate") != null)) {
            $.ajax({
                type: "GET",
...
$(document).ready(function() {
    ...
    $.cookie("Byecity_utm_term", $.url().param("utm_term"), { path: "/", domain: "byecity.com" });
    ...
    if ($.url().param("showhf") && !($.url().param("showhf") == undefined)) { //liuqing 2013-04-16
    ...
    if (document.URL.indexOf("1.byecity.com") == -1 && document.URL.indexOf("123.byecity.com") == -1) {
        //        var baseid = 0;
        //        var cid = 0;
        ...
        //        //99click 监测代码 只能加载一次，如果是跟踪日志的页面或者测试页面就不再加载
...
function setNavgator(url) {
    $(".nav a,.nav2_a a").each(function() {
        ...
        if (url.indexOf(href) == -1) {
            ...
            if (href.indexOf("visa123.byecity.com") > -1 || href.indexOf("visa1.byecity.com") > -1) {
                $("#liVisa").addClass("hover");
            }
        }
    });
...
$(function() {
    ...
    //头部右侧电话及微博微信按钮
    $("#logo_right").tabLayerBind({
        tabs: ">.tabLayer",
        curClass: "cur"
    });
    //首页用户登陆按钮
    $("#loginbox").tabLayerBind({
        tabs: ">.tabLayer",
        curClass: "cur"
    });
...
///手机验证码发送时间间隔
function checkByecityUserValidate(cookieName, df) {
    var result = "ok";
    var ByecityUser_ValidateCookie = $.cookie(cookieName);
    var d1 = new Date();
    if (ByecityUser_ValidateCookie == null)//没有cookie
    {
        $.cookie(cookieName, d1, { expires: 1, domain: 'byecity.com' });
    }
    else {
        var d2 = new Date(ByecityUser_ValidateCookie);
...
//ljh 20140125 顶部登陆
ByecityCommonTopLogin = function() {

    //头部频道导航选中效果
    (function() {
        var currentPinDao = window.location.host.split('.')[0];
        var pinDaoList$ = $('.bc_header .nav .nav_box ul.menu li');


        pinDaoList$.removeClass('cur'); //user,pay不选中
        //兼容www1,user123等
        for (var i = 0, len = pinDaoList$.length; i < len; i++) {
            if (currentPinDao.indexOf($(pinDaoList$[i]).attr('data-host')) === 0) {
                $(pinDaoList$[i]).addClass('cur');
                break;
            }
        }
    ...
    var loginSubmit$ = $('#byecity_top_login_submit'),
		loginOut$ = $('#byecity_top_login_loginout_url');

    //兼容静态页面获取顶部登陆状态
    (function() {
        var username = isLogin();
        if (username) {
            updateHeadAfterLogin(username);
        } else {
            updateHeadAfterLoginOut();
        }
    } ());

    function isLogin() {
        if (Cookie.Get('.ByecityCookie') || Cookie.Get('BtUser')) {
            return Cookie.Get('.bcusername');
        } else {
            return '';
        }
    }
    ...
    loginSubmit$.click(function() {
        ...
        var loginUrl = "ajax/UserHandler.ashx?method=login";

        var userName$ = $('#byecity_top_login_username'),
			username = $.trim(userName$.val()).dbc2sbc(),
			userNameDefaultValue = '请输入邮箱或手机号',
			pwd$ = $('#byecity_top_login_pwd'),
			pwd = $.trim(pwd$.val());

        var isRember = $('#byecity_top_login_isRember').attr('checked');

        var tips$ = $('#byecity_top_login_tip');

        //client验证
        var isHefa = false;
        (function() {

            if (userNameDefaultValue == username || !!!username) {
                tips$.html('<i class="tico"></i>请输入邮箱或手机号');
                tips$.show();
                userName$.focus();
                return;
            }
            ...
            if (!(username.isEmail() || username.isMobile())) {
                tips$.html('<i class="tico"></i>邮箱或手机号格式错误');
                tips$.show();
                userName$.focus();
                return;
            }
            ...
            if (!!!pwd) {
                tips$.html('<i class="tico"></i>请输入密码');
                tips$.show();
                pwd$.focus();
                return;
            }
            ...
            if (pwd.length < 6) {
                tips$.html('<i class="tico"></i>密码应该不小于6位');
                tips$.show();
                pwd$.focus();
                return;
            }
        ...
        $.ajax({
            url: loginUrl,
            data: { u: username, p: pwd, r: (isRember ? 1 : 0) },
            async: false,
            cache: false,
            success: function(data) {
                loginStatus = getLoginStaus(data);
                if (loginStatus.rsp_code !== 1) {
                    tips$.html('<i class="tico"></i>' + loginStatus.rsp_msg);
                    tips$.show();
                    pwd$.focus();
                    //todo
                } else if (loginStatus.rsp_code === 1) { //登陆成功

                    if (loginStatus.user.beAgent == "True")//是同行
                    {
                        var unionUrl = "http://union.byecity.com/UserTran.aspx?channel="
							+ loginStatus.user.channel + "&url=" + window.location.href;
                        //href = unionUrl;
                        window.location.href = unionUrl;
                    } else {
                        //不错处理
                    }

                    updateHeadAfterLogin(loginStatus.user.name);
                    /*$.cookie('.bcusername', loginStatus.user.name, { expires: 1, domain: 'byecity.com' });*/
    ...
    function getLoginStaus(loginRsp) {
        //历史原因,server没有返回JSON
        var arr = loginRsp.split(':');
        var status = arr[0],
			statusRsp;
        switch (status) {
            case '0':
                statusRsp = { rsp_code: 0, rsp_msg: "用户名或密码错误" };
                break;
            case '1':
                statusRsp = {
                    rsp_code: 1,
            ...
            case "3":
                ...
                break;
            default:
                statusRsp = { rsp_code: 9, rsp_msg: "服务器繁忙" };
                break;
        }
    ...
    //登陆后,更新头部登陆状态HTML
    function updateHeadAfterLogin(userName) {
        $("#byecity_top_login_loginS").hide();
        $("#byecity_top_login_regS").hide();
        $('#byecity_top_login_name').html('HI ' + userName + '...');
        $('#byecity_top_login_loginoutS').show();
    }

    //退出
    loginOut$.click(function() {
        var toUrl = window.location.href;
        var loginOutUrl = "ajax/UserHandler.ashx?method=loginout";
        $.ajax({
            url: loginOutUrl,
            async: false,
            cache: false,
    ...
    function updateHeadAfterLoginOut(returnUrl) {
        $("#byecity_top_login_loginS").show();
        $("#byecity_top_login_regS").show();
        $('#byecity_top_login_name').html('');
        $('#byecity_top_login_loginoutS').hide();
    }

}

var Cookie = Cookie || { //$.cookie莫名的不可用，所以添加这个
    //设置cookie
    Set: function(name, value, path, domain, time) {
        var expires = "";
        if (time) {
            var date = new Date();
