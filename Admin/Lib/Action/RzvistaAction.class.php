<?php
class RzvistaAction extends CommonAction{
	//认证列表
	...
	public function index() {
		//为类型搜索准备
		$this->assign('vcate',M('Rzvcate')->select());
		//列表过滤器，生成查询Map对象
		$map = $this->_search();
	...
	public function insert(){
		...
		$upload->savePath =  './Public/Admin/Images/Rzvista/';// 设置附件上传目录
		$upload->thumb = true;
		...
		if($j && $j>0)
		{
			...
			$data["pid"]=$id[$i]; 
			$data["price"]=$price[$i]; 
			$k=M('Rzvistainfo')->add($data);
			}
			if($k){
				$this->success(L('新增成功'));
			} else {
				//失败提示
				$this->error(L('新增失败').M('Rzvistainfo')->getLastSql());
			}
		}
	}
	...
	public function edit(){
		$this->assign('vcate',M('Rzvcate')->select());
		$this->assign('category',M('Rzcategory')->where('pid>0')->select());
		$model = D('Rzvista');
		$id=$_REQUEST['id'];
	...
	public function delete(){
		...
		unlink("__PUBLIC__/Admin/Images/Rzvista/s_{$logo}");
		unlink("__PUBLIC__/Admin/Images/Rzvista/m_{$logo}");
		//删除产品的同时删除相关的办理地区信息
